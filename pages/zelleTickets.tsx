  ...
  const Dashboard = () => {
    ...
    const fetchTickets = async () => {
      try {
        ...
        const zelleTickets = data.filter((ticket: Ticket) => ticket.paymentType === 'zelle');
        setTickets(zelleTickets);
      } catch (error) {
        ...
        if (isConfirmed) {
          ...
          try {
            const response = await fetch('https://new-api.worldeventaccess.com/api/Ticket/ConfirmTicketsGroup', {
              ...
              body: JSON.stringify(groupId.replace(/['"]+/g, '')),
            });
